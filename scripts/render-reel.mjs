...
import { unknownKeys, unknownKeysMessage } from "./post-keys.mjs";
import {
  cueFramesOf,
  layoutOfBeat,
...
// own beat's length.
//
// There used to be a cross-check here that laid every reel out a second time
// and threw on a frame of disagreement. Once shots went through this call it
// compared the same code with itself, and the beat half summed the same
// integers twice, so it moved to scripts/presets.test.mjs (the TL_REEL and
// TL_ANCHORED_REEL assertions, numbers taken off a real render) on 2026-09-16.
...
for (const name of names) {
  ...
  // A key nothing reads is refused before anything else looks at the post,
  // probes included: a misspelt `motion` renders clean and wrong, and that is
  // not a writing gate to skip on a sheet but a spec that does not say what
  // its author thinks. Checked after presets, so a preset's own keys count.
  const unknown = unknownKeys({ ...post, reel });
  if (unknown.length) throw new Error(unknownKeysMessage(name, unknown));
